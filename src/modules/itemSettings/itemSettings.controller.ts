...
import validate from '@/modules/itemSettings/itemSettings.validation'
...
@Service()
export default class ItemSettingsController implements IAppController {
  public path = '/item-settings'
  ...
  private create = async (
    ...
    try {
      ...
      const response = await this.itemSettingsService.create(approval, +fee)
  ...
  private update = async (
    ...
    try {
      ...
      const response = await this.itemSettingsService.update(approval, +fee)
  ...
  private fetch = async (
    ...
    try {
      const response = await this.itemSettingsService.fetch()
      res.status(200).json(response)
    } catch (err: any) {
      next(new HttpException(err.status, err.message, err.statusStrength))
    }
